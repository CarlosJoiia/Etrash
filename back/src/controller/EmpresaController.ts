import { PrismaClient, empresas } from "@prisma/client";
...
export async function saveEmpresa(request: Request, response: Response) {
  const empresas = request.body;

  const stringcnpj = String(empresas.cnpj);
  ...
  if (data.status === "OK") {
    ...
    if (verificarCooperativa) {
      if (verificarCooperativa.email === empresas.email) {
        return response
          .status(400)
          .json({ mensagem: "Este E-mail já está registrado." });
      } else if (verificarCooperativa.contact === empresas.contact) {
        return response
          .status(400)
    ...
    if (verificarEmpresa) {
      if (verificarEmpresa.email === empresas.email) {
        return response
          .status(400)
          .json({ mensagem: "Este E-mail já está registrado." });
      } else if (verificarEmpresa.contact === empresas.contact) {
        return response
          .status(400)
          .json({ mensagem: "Este Contato já está registrado." });
      } else {
    ...
    const verificarPessoa = await prisma.pessoafisica.findFirst({
      where: {
        OR: [{ email: empresas.email }, { contact: empresas.contact }],
      },
    });

    if (verificarPessoa) {
      if (verificarPessoa.email === empresas.email) {
        return response
          .status(400)
          .json({ mensagem: "Este E-mail já está registrado." });
      } else {
  ...
  return (
    await response
      .status(201)
      .json({ mensagem: `${empresas.name} Foi Cadastrado.` }),
    prisma.empresas.create({
      data: empresas,
    })
  );
}

export async function ValidarEmprersa(request: Request, response: Response) {
  const empresas = request.body;
  ...
  const token = jwt.sign({ email: empresas.email }, KEYcodi, {
...
export async function Validacao(request: Request, response: Response) {
  ...
  try {
    ...
    if (!Empresas) {
      return response
        .status(404)
        .json({ mensagem: "empresas não encontrada." });
    }

    // Verifica se o status atual é "NaoVerificado"
    if (Empresas.status === "NaoVerificado") {
      const EmpresasAtualizada = await prisma.empresas.update({
        where: {
          email: email,
        },
        data: {
